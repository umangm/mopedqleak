...
void init_ir() {
	...
	//type_array[IR_FLOAT] = Strdup("floating point");
		...
		if (ret_dim) {
			...
			if (dir == 1) {
				irInsertRetVar(default_return_name_bool_array, 
					IR_BOOL, ret_dim, 0);
				module->retvar->name = default_return_name_bool_array;	//Added by Umang
		...
		/* if the return var is NOT an array */
			...
			if (dir == 1) {
				irInsertRetVar(default_return_name_bool, 
					IR_BOOL, NULL, 0);
				module->retvar->name = default_return_name_bool;
	...
	default: {
		...
		if (ret_dim) {
			...
			if (dir == 1) {
				...
				module->retvar->name = default_return_name_array;
		...
		/* if the return var is NOT an array */
			...
			if (dir == 1) {
				...
				module->retvar->name = default_return_name;
...
int irIsReturnInside(IrStmt stmt) {
	...
	else if (stmt->type == IR_P) {
		
		IrPchoice pchoice = (IrPchoice) stmt->content;
		for (; pchoice; pchoice = pchoice->next) {
			
			IrStmt pchoiceStmt = pchoice->stmt;
			for (; pchoiceStmt; pchoiceStmt = pchoiceStmt->next) {
				if (irIsReturnInside(pchoiceStmt))
					return 1;
			}
		}
	}
...
int hasSameLabels(IrStmt stmt1, IrStmt stmt2) {
	...
	else if (stmt1->type == IR_P) {
		IrPchoice pchoice = (IrPchoice) stmt1->content;
		for (; pchoice; pchoice = pchoice->next) {
			if (hasSameLabels(pchoice->stmt, stmt2)) {
				return 1;
			}
		}
	}
	...
	else if (stmt2->type == IR_P) {
		IrPchoice pchoice = (IrPchoice) stmt2->content;
		for (; pchoice; pchoice = pchoice->next) {
			if (hasSameLabels(stmt1, pchoice->stmt)) {
				return 1;
			}
		}
	}
			...
			for (; pchoice; pchoice = pchoice->next) {
				if (pchoice->stmt) {
					if (pchoice->stmt->type == IR_IF || pchoice->stmt->type == IR_DO || pchoice->stmt->type == IR_P) {
						if (irLabelExist(pchoice->stmt, label)) return 1;
					}
					else {
						if (irLabelExist(pchoice->stmt->next, label)) return 1;
					}
				}
			}
		}
...
void irLabelUpdate(IrStmt stmt) {
        ...
        for (; stmt; stmt = stmt->next) {
				...
				if ((stmt->type != IR_IF) && (stmt->type != IR_DO) && (stmt->type != IR_P)) continue;
                ...
                if ((stmt->type == IR_IF) || (stmt->type == IR_DO)){
					...
					IrPchoice pch = (IrPchoice) stmt->content;
					for (; pch; pch = pch->next) {
						if (!pch->stmt) continue;
						if ((pch->stmt->type == IR_IF) || (pch->stmt->type == IR_DO) || (pch->stmt->type == IR_P)) {
							irLabelUpdate(pch->stmt);
						}
						
						else {
						/* the first statement of ifdo statement 
				   		always has the same label as the stmt */
							pch->stmt->label = stmt->label;
							irLabelUpdate(pch->stmt->next);
						}
					}
				}
        	}
...
IrPchoice irPchoiceCreate(float probability, IrStmt stmt, IrPchoice next) {
	
	IrPchoice pchoice = Malloc(sizeof(struct IrPchoiceStruct));
	pchoice->probability = probability;
	pchoice->stmt = stmt;
	pchoice->next = next;
...
void irPchoiceFree(IrPchoice pchoice) {
	
	if (pchoice->next) irPchoiceFree(pchoice->next);

	//irFnodeFree(pchoice->cond);
	irStmtFree(pchoice->stmt);
	Free(pchoice);
...
void irStmtFree(IrStmt stmt) {
	...
	switch (stmt->type) {
		...
		case IR_P:
			irPchoiceFree((IrPchoice) stmt->content);
			break;
...
void irFree() {
	...
	#ifdef ddb
	printf("\nPrinting ir_ret before freeing\n");
	
	IrGbl v = ir_ret;
	while(v){
		if(v->var){
			//printf("name = %s, type = %d, dim = (%d,%d,%d), bits = %d\n", wIdentString(v->var->name), v->var->type, v->var->dim->len, v->var->dim->hi, v->var->dim->lo, v->var->bits);
			printf("name = %s\n", wIdentString(v->var->name));
			printf("type = %d\n",v->var->type);
			printf("bits = %d\n", v->var->bits);
			if(v->var->dim) printf("dim = (%d,%d,%d)\n", v->var->dim->len, v->var->dim->hi, v->var->dim->lo);
		}
		//printf("\n");
		v = v->next;
	}
