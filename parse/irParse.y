...
%token INIT
%token ARROW
%token IF FI ELSE DO OD PCHOICE CHOICEP
...
%type <y_ifdo> ifclnstmtlist
%type <y_ifdo> doclnstmtlist
//%type <y_ifdo> clnstmt
%type <y_pchoice> pchoicestmt
%type <y_pchoice> pstmtlist
%type <y_pchoice> pstmt
...
automaton: defines gbldecls /*initconfig*/ /*stmtlist*/ modulelist {
        /*
        IrStmt ml;
...
lblstmt: IDENT ':' stmt {
        if($1==wIdentCreate((char*)"main")) irerror("Label cannnot have name \"main\"");
		$3->label = $1;
		$$ = $3;
	}
	| stmt {
...
stmt: rawtrans label optboolexpr {
	...
	| pchoicestmt optsemicolon {
		$$ = irStmtCreate(IR_P, 0, $1, NULL);
	}
	| action ';' { $$ = $1; }
	| assgnlist ';' {
...
pchoicestmt:	PCHOICE pstmtlist CHOICEP {
		pchoice_found = 1;
		if(!$2) irerror("empty pchoice statement");
		float prob_val = 0.0;
		IrPchoice irp = $2;
		while(irp){
...
*/
ifclnstmtlist: DOUBLECOLON boolexpr CONDARROW stmtlist DOUBLECOLON ELSE CONDARROW stmtlist {
        ...
        if (!$8) irerror("invalid statement");
		...
		$$->next = irIfdoCreate(irFnodeCreate(IR_ELSE, 0 ,NULL, NULL), $8, NULL);
    };
    
doclnstmtlist: DOUBLECOLON boolexpr CONDARROW stmtlist {
        if (!$4) irerror("invalid statement");
		// handles the case ":: cond -> x: ...;" by adding "skip"
		if ($4->label) {
			IrStmt s = irStmtCreate(IR_SKIP, 0, NULL, NULL);
		...
		$$->next =  irIfdoCreate(irFnodeCreate(IR_ELSE, 0 ,NULL, NULL), irStmtCreate(IR_BREAK, 0, NULL, NULL), NULL) ;
    };
...
pstmt: DOUBLECOLON FLOAT CONDARROW stmtlist {
		if (!$4) irerror("invalid statement");
		if ($2>1 || $2<0) irerror("invalid value of probability");
		if ($4->label) {
			IrStmt s = irStmtCreate(IR_SKIP, 0, NULL, NULL);
			s->next = $4;
			$4 = s;
		}
		$$ = irPchoiceCreate($2, $4, NULL);
	}
...
modulehead: MODULE rettype IDENT '(' params ')' {
		if($3==wIdentCreate((char*)"main")) main_found=1;
		
		if($3==wIdentCreate((char*)"main") && $5) irerror("Main cannot have arguments");
